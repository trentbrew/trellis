...
| --- | --- | --- |
| Kernel / CLI | [`trellis`](https://www.npmjs.com/package/trellis) | 4.0.0 |
...
### Git-authoritative file tier (ADR 0038)

**Git is the sole authority over file bytes. The op-log can no longer write file state.**

The `9801056`-class silent revert — where `git sync` materialized stale op-log bytes over newer disk content — is fixed by removing the second authority, not by adding another guard. Two systems claiming authority over the same bytes is structurally unsound; ADR 0038 makes git the only writer and the op-log a pure provenance layer *incapable* of clobbering disk. See [Write path](/architecture/write-path).

- **`git sync` commits the working tree as-is.** No more blob-store materialization — it checks out `-B`, stages, and commits the actual bytes on disk.
- **`vcs:gitSync` annotations.** Each sync records `{ gitCommitHash, gitBranch }` on the causal stream. The op-log holds provenance — milestones, decisions, lane lifecycle — never byte state.
- **Promote delivers bytes via git merge**, then rechains annotation ops onto integration. Real merge conflicts abort before replay.
- **Lane worktrees are git checkouts.** Entering a lane auto-commits the agent's working bytes onto the lane branch; `lanes.worktreeBind` isolates agents so one's `revert`/`reset` cannot touch another's files.

### Breaking changes

- `GitSyncResult.refused` / `reason` removed — sync never refuses; the CLI refusal branches in `git sync` and milestone auto-commit are gone.
- `syncIntegrationToGit` drops `blobResolver`, `integrationOps`, `headOpHash`; `resolveIntegrationHead` removed from the `git-sync` module.
- `trellis git sync` no longer journals previously-unjournaled edits — disk edits flow through git commits, not file-materializing ops.

### Fixes

- `trellis lane list -p <dir>` resolved from cwd instead of the target (Commander's duplicate `-p` shadowed the parent path); the list action now resolves like every other `lane` subcommand.
- workflow-pipeline-primitives test mock brought in line with the real `EAVStore` API.
