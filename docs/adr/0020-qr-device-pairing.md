...
the same identity. Magic email is appropriate for hosted Studio *accounts*; it
...
`identityEntityId`, signed by an already-authorized key (bootstrap: the identity root key on Device A).,
...
| **1** | QR encode helpers (`renderPairingQr` / terminal CLI) + optional `127.0.0.1` listener; camera decode = Phase 2 surface                |
