...
import { registerCloneCommands } from './clone-cli.js';
...
// ---------------------------------------------------------------------------
// trellis git journal catch-up — absorb working-tree drift into the op-log
// ---------------------------------------------------------------------------

gitCmd
  .command('journal')
  .description('Op-log journal operations')
  .command('catch-up')
  .description(
    'Scan the working tree and journal any disk→op-log divergence as ops (refuses silently if unreconcilable)',
  )
  .option('-p, --path <path>', 'Repository path', '.')
  .option('-b, --branch <name>', 'Branch to advance head to last journaled op')
  ...
  .action(async (opts) => {
    ...
    if (opts.json) {
      ...
      if (result.journaled > 0) {
        console.log(
          chalk.green(`✓ Journaled ${result.journaled} op(s) from working tree`),
        );
      } else {
        console.log(chalk.dim('Working tree already matches op-log'));
      }
      if (result.unreconciled.length > 0) {
        console.error(
          chalk.yellow(
            `⚠ ${result.unreconciled.length} path(s) unreconciled: ${result.unreconciled.join(', ')}`,
          ),
        );
        process.exit(1);
      }
